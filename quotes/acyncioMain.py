...
async def get_authors(base_url, soup):
    authors = []
    ...
    for element in quote_elements:
        ...
        author_data = await get_author_data(base_url + author_link)
        if author_data:
...
async def get_quotes(base_url, soup):
    quotes = []
    quote_elements = soup.select('div.quote')
...
async def get_urls(base_url):
    ...
    async with aiohttp.ClientSession() as session:
        while urls1:
            url = urls1.pop(0)
            html = await fetch_html(session, base_url + url)
            soup = BeautifulSoup(html, 'html.parser')

            data_authors.extend(await get_authors(base_url, soup))
        ...
        while urls2:
            ...
            data_quotes.extend(await get_quotes(base_url, soup))
...
async def main():
    try:
        data_authors, data_quotes = await get_urls(BASE_URL)
